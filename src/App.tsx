...
function App() {
  ...
  const loadLevel = useCallback(async (levelId: number) => {
    ...
    setError(null);
    try {
      setGame(await startLevel(levelId));
    } catch (cause) {
  ...
  const handleCell = async (cellIndex: number) => {
    ...
    setError(null);
    try {
      // Cell indexes sent to the authoritative engine are 1-based.
      setGame(await applyMove(game.session_id, game.move_count + 1, 'clear_group', cellIndex + 1));
    } catch (cause) {
  ...
  const movesRemaining = Math.max(0, 20 - (game?.move_count ?? 0));
            ...
            {board.map((value, index) => {
              ...
              return (
                <button
                  key={index}
                  className="tile"
                  style={{ '--tile-color': color ? palette[color] : 'transparent' } as CSSProperties}
