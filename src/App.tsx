...
const seed: TileColor[] = [
  'coral', 'coral', 'sun', 'lagoon', 'lagoon', 'azure',
  'violet', 'coral', 'sun', 'sun', 'lagoon', 'azure',
  'violet', 'violet', 'mint', 'azure', 'azure', 'coral',
  'mint', 'sun', 'lagoon', 'lagoon', 'violet', 'coral',
  'azure', 'mint', 'mint', 'sun', 'coral', 'azure',
  'violet', 'lagoon', 'sun', 'sun', 'mint', 'coral',
  'azure', 'azure', 'violet', 'mint', 'lagoon', 'coral',
  'sun', 'mint', 'violet', 'azure', 'lagoon', 'coral',
  'coral', 'sun', 'sun', 'lagoon', 'mint', 'azure',
  'violet', 'violet', 'azure', 'mint', 'coral', 'lagoon',
  'sun', 'azure', 'lagoon', 'mint',
];
...
function App() {
  ...
  const groups = useMemo(() => {
    ...
    tiles.forEach((tile, index) => {
      const group = result.get(tile.color) ?? [];
      group.push(index);
      result.set(tile.color, group);
    });
    return result;
  }, [tiles]);
  ...
  const handleTile = (index: number) => {
    ...
    const group = groups.get(tile.color) ?? [];
  ...
  return (
    <main className="game-shell">
      <header className="hud">
        ...
        <div className="resource-pill" aria-label="Coins">
        ...
        <div className="board">
          {tiles.map((tile, index) => (
            <button
              ...
              style={{ '--tile-color': palette[tile.color] } as React.CSSProperties}
              onClick={() => handleTile(index)}
              aria-label={`Color tile ${tile.color}`}
            >
              <span />
            </button>
          ))}
      ...
      <footer className="controls">
        ...
        <button className="primary-button" onClick={() => { setLevel((value) => value + 1); resetLevel(); }}>
          Next level
        </button>
      </footer>
