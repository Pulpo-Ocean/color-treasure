...
function GameScreen({ onBack }: { onBack: () => void }) {
  ...
  const loadLevel = useCallback(async (levelId: number) => {
    ...
    try { setGame(await startLevel(levelId)); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to start the level.'); } finally { setLoading(false); }
  }, []);
  useEffect(() => { void loadLevel(1); }, [loadLevel]);
  const handleCell = async (cellIndex: number) => {
    if (!game || busy || game.result !== 'CONTINUE') return;
    setBusy(true); setError(null);
    try { const special = specialAt(game.special_state, cellIndex + 1); setGame(await applyMove(game.session_id, game.move_count + 1, special ? 'activate_special' : 'clear_group', cellIndex + 1)); }
  ...
  return <main className="app-shell game-shell">
    <header className="hud"><button className="icon-button" onClick={onBack} aria-label="Back to home">‹</button><div><span className="eyebrow">COLOR TREASURE</span><h1>Coral Garden</h1></div><div className="resource-pill" aria-label="Coins"><span className="coin">◆</span><strong>—</strong></div></header>
    ...
    {resultLabel && <div className="result-banner" role="status">{resultLabel}</div>}{error && <div className="error-banner" role="alert">{error}</div>}
...
const screenMeta: Record<Exclude<AppScreen, 'home' | 'game'>, { title: string; subtitle: string }> = {
  map: { title: 'World Map', subtitle: 'Explore the five treasure zones.' }, collection: { title: 'Your Crew', subtitle: 'Collect and grow your sea friends.' }, missions: { title: 'Missions', subtitle: 'Complete objectives and earn rewards.' }, events: { title: 'Events', subtitle: 'Limited-time treasure adventures.' }, shop: { title: 'Shop', subtitle: 'Spend resources when you choose.' },
};

function App() {
  const [screen, setScreen] = useState<AppScreen>('home');
  const navigate = (next: AppScreen) => setScreen(next);
  const content = screen === 'home' ? <HomeScreen onNavigate={navigate} onPlay={() => navigate('game')} /> : screen === 'game' ? <GameScreen onBack={() => navigate('home')} /> : <PlaceholderScreen {...screenMeta[screen]} onBack={() => navigate('home')} onNavigate={navigate} />;
