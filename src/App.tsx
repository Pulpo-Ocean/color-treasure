...
const palette: Record<TileColor, string> = {
  coral: '#ff6b6b', sun: '#ffd166', lagoon: '#2ec4b6', violet: '#9b5de5', mint: '#80ed99', azure: '#4dabf7',
};
...
const OBJECTIVE_LABELS: Record<string, string> = {
  clear_tiles: 'Clear tiles', collect_shells: 'Collect shells', unlock_treasure: 'Unlock treasure',
  rescue_creatures: 'Rescue creatures', break_coral: 'Break coral', mixed: 'Complete objectives',
};
...
function App() {
  ...
  const handleCell = async (cellIndex: number) => {
    ...
    try { setGame(await applyMove(game.session_id, game.move_count + 1, 'clear_group', cellIndex + 1)); }
  ...
  return (
    <main className="game-shell">
      <header className="hud">
        ...
        <div className="resource-pill" aria-label="Coins"><span className="coin">◆</span><strong>—</strong></div>
      </header>
      <section className="level-card" aria-label="Level information">
        <div><span className="label">LEVEL</span><strong>{level}</strong></div>
        <div className="objective"><span className="shell">◈</span><span>{objectiveLabel}{target ? ` ${progress}/${target}` : ''}</span></div>
      ...
      <section className="board-wrap" aria-label={`Level ${level} game board`} aria-busy={loading || busy}>
        {loading ? <div className="game-message">Loading level…</div> : (
          <div className="board">
            {board.map((value, index) => {
              const color = colorByValue[value];
              return <button key={index} className="tile" style={{ '--tile-color': color ? palette[color] : 'transparent' } as CSSProperties}
                onClick={() => void handleCell(index)} disabled={busy || !color || game?.result !== 'CONTINUE'}
                aria-label={color ? `Color tile ${color}, cell ${index + 1}` : `Empty cell ${index + 1}`}><span /></button>;
            })}
          </div>
        )}
      </section>
      ...
      <footer className="controls">
        ...
        <button className="primary-button" onClick={() => void loadLevel(level + 1)} disabled={loading || busy || game?.result !== 'WIN' || level >= 100}>Next level</button>
      </footer>
      <p className="status">Server-authoritative gameplay • board, moves, objectives and rewards are validated by Color Treasure.</p>
    </main>
  );
}
